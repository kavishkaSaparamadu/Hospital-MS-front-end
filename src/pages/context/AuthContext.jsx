...
const getStoredUser = () => {
  //retrieve the user data from the localStorage
  try {
...
const AuthReducer = (state, action) => {
  //state (representing the current state) and action (representing the type of action dispatched to update the state)
  switch (action.type) {
    case "LOGIN_START":
      return {
        user: null,
...
export const AuthContextProvider = ({ children }) => {
  //"AuthContextProvider" component of the application to enable state management with the "AuthContext" and "AuthReducer".
  ...
  useEffect(() => {
    //useEffect hook is used to automatically update the localStorage whenever the state.user changes
